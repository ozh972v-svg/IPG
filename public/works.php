...
/** Парсер через SimpleXML — правильно читает вложенный <Parent> */
...
/** Fallback парсер — через регекспы (плоский, без parent_code) */
...
function do_sync_works(PDO $pdo): array {
    ...
    if (count($parsed) === 0) {
        return ['total' => 0, 'message' => 'Записей 0. Длина ответа: ' . strlen($xml) . ' байт. Парсер: ' . $method];
    }
...
<div class="container">
  ...
  <div class="card">
    ...
    <div class="btn-row">
      <a href="sync.php?type=works&run=1" class="btn btn-green" onclick="return confirm('Обновить работы? Может занять 3-5 минут. Не закрывайте вкладку.')">🔧 Обновить работы</a>
    ...
    <p style="font-size:13px;color:#666;margin-top:12px;">
      Таймаут 300 сек. Парсер: SimpleXML (правильно читает вложенный Parent), fallback — regex.
    </p>
  </div>
