...
if (!empty($_GET['find_prereq'])) {
    ...
    $res = $stmt->fetchAll();
    // Ограничим 15 после дедупликации
...
// Постраничный список работ (с дедупликацией по operation_code)
...
/* ===== КАТЕГОРИИ ВНУТРИ ГРУППЫ (со счётчиками, с дедупликацией) ===== */
...
/* ===== КАТЕГОРИИ ВООБЩЕ (без группы, с дедупликацией) ===== */
...
$stmt = $pdo->prepare("
    SELECT COUNT(*) FILTER (WHERE it_is_group = TRUE AND deleted = FALSE AND model = :m) AS groups,
           COUNT(DISTINCT operation_code) FILTER (WHERE it_is_group = FALSE AND deleted = FALSE AND model = :m) AS works
    FROM work_operations
");
$stmt->execute([':m' => $model]);
$stats = $stmt->fetch();
