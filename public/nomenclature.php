...
if ($q !== '') {
    $where[] = "(code ILIKE :q OR name ILIKE :q OR full_name ILIKE :q OR eng_name ILIKE :q OR code_1c ILIKE :q)";
    $params[':q'] = '%' . $q . '%';
}
if ($parent !== '') {
    $where[] = "parent = :p";
...
$stmt = $pdo->prepare("SELECT COUNT(*) FROM nomenclatures $whereSql");
$stmt->execute($params);
...
$stmt = $pdo->query("SELECT COUNT(*) FROM nomenclatures WHERE deleted = FALSE");
$totalAll = (int)$stmt->fetchColumn();

$stmt = $pdo->query("SELECT MAX(updated_at) FROM nomenclatures");
$lastSync = $stmt->fetchColumn();

// Список групп (parent) для выпадающего фильтра
$parents = $pdo->query("
    SELECT DISTINCT parent FROM nomenclatures
    WHERE parent IS NOT NULL AND parent <> '' AND deleted = FALSE
    ORDER BY parent
...
<title>Поиск по номенклатуре</title>
<style>
  * { box-sizing: border-box; -webkit-tap-highlight-color: transparent; }
  ...
  .search-bar select { padding:12px; border:1.5px solid #e5e7eb; border-radius:10px; font-size:14px; font-family:inherit; max-width:250px; }
...
<div class="container">
  ...
  <div class="card">
    ...
    <div class="btn-row">
      ...
      <?php if ($user['is_admin']): ?>
        <a href="sync.php?type=nomenclature" class="btn btn-small">🔄 Синхронизация с 1С</a>
      <?php endif; ?>
    </div>
  </div>
  ...
  <?php if ($totalAll === 0): ?>
    <div class="warn">
      ⚠️ Справочник номенклатуры пока пуст.
      <?php if ($user['is_admin']): ?>
        Перейдите в <a href="sync.php?type=nomenclature">синхронизацию</a> и запустите обновление.
      <?php else: ?>
  ...
  <div class="card">
    <form method="get">
      <div class="search-bar">
        <input type="text" name="q" value="<?= e($q) ?>" placeholder="Поиск по коду, наименованию, чертёжному номеру…">
        <?php if ($parents): ?>
          <select name="parent">
            <option value="">Все группы</option>
            <?php foreach ($parents as $p): ?>
              <option value="<?= e($p) ?>" <?= $parent === $p ? 'selected' : '' ?>><?= e($p) ?></option>
            <?php endforeach; ?>
          </select>
        <?php endif; ?>
        <button type="submit" class="btn">Найти</button>
        <?php if ($q !== '' || $parent !== '' || $showDeleted): ?>
          <a href="nomenclature.php" class="btn btn-secondary">Сбросить</a>
        <?php endif; ?>
      </div>
        ...
        <tbody>
          <?php foreach ($rows as $r): ?>
            <tr>
              <td>
                <code><?= e($r['code']) ?></code>
                <?php if ($r['deleted']): ?><span class="badge badge-red">Удалён</span><?php endif; ?>
              </td>
              <td>
                <?= e($r['name'] ?: '—') ?>
                <?php if ($r['eng_name']): ?>
              ...
              <td style="max-width:400px;font-size:12px;color:#555;"><?= e($r['full_name'] ?: '—') ?></td>
              <td><?= e($r['parent'] ?: '—') ?></td>
              <td><?= e($r['base_measure'] ?: '—') ?></td>
              <td><?= $r['code_1c'] ? '<code>' . e($r['code_1c']) . '</code>' : '—' ?></td>
            </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
