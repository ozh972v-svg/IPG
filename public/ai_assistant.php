...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['q'])) {
    ...
    /* ============================================================
       ...
       ВАЖНО: не фильтруем жёстко по complectation — теряются работы,
       у которых шасси записано иначе (в поле model, в code с @ и т.п.).
       Вместо этого даём приоритет работам нужного шасси при сортировке.
       ============================================================ */
    $works = [];
    ...
    if ($canSearchWorks) {
        ...
        if (!empty($keywords) && !empty($works)) {
            foreach ($works as &$w) {
                ...
                /* Приоритет работам нужного шасси */
            ...
            /* Отбрасываем работы с нулевым счётом — нерелевантные */
    ...
    $system = "Ты — помощник мастера-приёмщика сервиса КАМАЗ/КОМПАС.\n"
            ...
            . "ВАЖНО: работы идут по бренду целиком, не только по указанному шасси. Если в названии есть подходящее — оно там есть, ищи внимательно.\n"
            . "\n"
            . "ПРАВИЛА ОТВЕТА:\n"
            . "1. Сначала ищи ответ в предоставленном контексте. Если есть подходящие работы — перечисли их с кодами и названиями.\n"
            . "2. НЕ ГОВОРИ «точного совпадения нет», если есть работа с совпадающим ключевым словом в названии (например, «Замена генератора» по запросу «замена генератора»). Просто перечисли её.\n"
            . "3. Если в контексте нет точного совпадения, но есть близкое по смыслу — предложи его и объясни, почему.\n"
            . "4. Если в контексте совсем ничего нет, но ты знаешь ответ — ответь и начни со строки «⚠️ Общий ответ (в базе по этому запросу ничего не найдено):».\n"
            . "5. Не выдумывай коды операций, номера РА или VIN, которых нет в контексте.\n"
            . "6. Отвечай кратко, по-русски.\n"
            . "7. Если в вопросе спрашивают про фото — используй данные «фото: N» у записей РА.";
